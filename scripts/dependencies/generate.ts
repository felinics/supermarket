...
for (const [id, recipe] of Object.entries(definitions)) {
  ...
  const manifest = {
    ...
    scripts: { install: 'install.sh', update: 'install.sh', remove: 'remove.sh', check_update: 'check-update.sh', version: 'version.sh' },
  ...
  for (const [file, action] of [['install.sh', 'install'], ['check-update.sh', 'check_update'], ['version.sh', 'version']]) {
