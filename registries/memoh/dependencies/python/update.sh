# shellcheck shell=sh
# Update the CPython overlay: install versions/<version> on top of the image baseline, then switch `current`.
#
...
# release; "3.13.2" is exact), MEMOH_DEP_CURRENT_VERSION, MEMOH_DEP_RESULT.
# The interpreter is downloaded by uv (requires: [uv], the image baseline or
# the remote machine's own copy), so UV_PYTHON_INSTALL_MIRROR and the other
# UV_* variables apply when the Server exports them. The previous version
# stays under versions/ for rollback. Never hard-code the workspace data mount
# path (WD-EXEC-001).
