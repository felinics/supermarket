...
# MEMOH_DEP_OS / _ARCH / _LIBC, MEMOH_DEP_RESULT, NODEJS_MIRROR and
# NODEJS_MUSL_MIRROR (§5.4, same defaults as docker/toolkit/install.sh). The
# previous version stays under versions/ for rollback. Never hard-code the
# workspace data mount path (WD-EXEC-001).
