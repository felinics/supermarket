...
# Native npm releases use postinstall only to place the optional platform
# binary. Link that known file ourselves; older JavaScript releases already
# expose their executable through npm's regular bin link.
case "$MEMOH_DEP_ARCH" in
  amd64) npm_arch=x64 ;;
  arm64) npm_arch=arm64 ;;
  *) dep_log "unsupported Claude Code architecture"; exit 1 ;;
esac
platform="$MEMOH_DEP_OS-$npm_arch"
for native in \
  "$stage/root/lib/node_modules/$pkg/node_modules/$pkg-$platform/claude" \
  "$stage/root/lib/node_modules/$pkg-$platform/claude"; do
  if [ -f "$native" ]; then
    chmod 755 "$native"
    mkdir -p "$stage/root/bin"
    rm -f "$stage/root/bin/claude"
    ln -s "../${native#"$stage/root/"}" "$stage/root/bin/claude"
    break
  fi
done
