...
- Use the selected Memoh workspace and its network settings. Report a network failure without changing workspace permissions.
...
If deployment fails due to network issues, inspect the selected workspace network configuration and report the concrete failure. Do not assume a Codex-specific permission escalation mechanism exists in Memoh.
